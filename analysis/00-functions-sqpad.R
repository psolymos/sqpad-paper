...
.sqpad.fit <- function(
    ...
    if (type == "conv") {
        ...
        opt <- optim(init, nll, method = method, hessian = hessian,
    ...
    if (type == "approx") {
        ...
        if (is.null(K)) {
            nll <- function(parms, Y, A, dur, dis, X, Z, det, distcorr, montecarlo, dij) {
            ...
            nll <- function(parms, Y01, A, dur, dis, X, Z, det, distcorr, montecarlo, dij) {
                D.hat <- exp(drop(X %*% parms[ix]))
                delta <- exp(drop(Z %*% parms[iz]))
                tau <- exp(parms[ma])
    ...
    if (type == "full") {
        ...
        if (is.null(K)) {
            nll <- function(parms, Y, A, dur, dis, X, Z, Nmax, det, distcorr, montecarlo, dij) {
                D.hat <- exp(drop(X %*% parms[ix]))
                delta <- exp(drop(Z %*% parms[iz]))
                tau <- exp(parms[ma])
            ...
            YK <- Y
            YK[YK > K] <- K
            nll <- function(parms, YK, A, dur, dis, X, Z, K, Nmax, det, distcorr, montecarlo, dij) {
                D.hat <- exp(drop(X %*% parms[ix]))
                delta <- exp(drop(Z %*% parms[iz]))
                tau <- exp(parms[ma])
    ...
    if (type == "tt1") {
        ...
        nll <- function(parms, Y, A, tt1, dur, dis, X, Z, Nmax, distcorr, montecarlo, dij) {
            D.hat <- exp(drop(X %*% parms[ix]))
            delta <- exp(drop(Z %*% parms[iz]))
            tau <- exp(parms[ma])
...
sqpad <- function (formula, data, dis, dur,
    ...
    if (missing(data))
        data <- NULL
    d <- detect:::svisitFormula(formula, data, n=0) ## global environment
    Y <- d$y
    X <- d$x$sta
    Z <- d$x$det
    Xlevels <- d$levels
