...
- `00-functions-sqpad.R`: functions implementing the SQPAD estimating procedures (these are also part of the detect package)
...
- extraDistr: implementing the triangular distribution that SQPAd uses

```{r eval=FALSE}
install.packages(
    pkgs = c('bSims', 'detect', 'qs', 'rconfig', 'mefa4', 
...
Methods to extract log-likelihood and related information criteria:
...
yydis <- as.matrix(Xtab(Count ~ UniqueID + DISTANCE, 
    x[x$SPECIES == spp,]))
yydis <- yydis[rn,c("0-49 m", "50-100 m")]
Y <- rowSums(yydis)
...
- `D`: value for the population density (individuals/ha)
...
sptable <- detect::paired |> 
    filter(!duplicated(SPECIES) & SPECIES %in% xf$species) |> 
    arrange(SPECIES) |>
    mutate(`Species Code`=SPECIES, `Species Name`=SpeciesName) |>
    select(`Species Code`, `Species Name`)
knitr::kable(sptable, digits = 3, caption = "Species used in field data analysis")
```
